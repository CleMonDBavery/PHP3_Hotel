...
<div class="content-wrapper">
    <form method="post" class="container-fluid" enctype="multipart/form-data"
        action="{{ route('service.update', $services->id) }}">
        @csrf
        @method('PATCH')
            ...
            <div class="card-body">
                <div class="form-group">
                    ...
                    <input type="text" class="form-control @error('name') is-invalid @enderror" id="name"
                        placeholder="Nhập dịch vụ" name="name" value="{{ old('name', $services->name) }}">
                    @error('name')
                ...
                <div class="form-group">
                    ...
                    <input type="number" step="0.01" class="form-control @error('price') is-invalid @enderror"
                        id="price" placeholder="Giá" name="price" value="{{ old('price', $services->price) }}">
                    @error('price')
                        <span class="invalid-feedback">{{ $message }}</span>
                    @enderror
                </div>
